...
#include "../src/talkTUI.h"

/* Global UI struct */
ui_t u;

/* Functions that generate box contents */
void text(ui_box_t *b, char *out) { sprintf(out, "%s", (char *)b->data1); }
...
void draw(ui_box_t *b, char *out) {
  ...
  for (y = 0; y < b->h; y++) {
    for (x = 0; x < b->w; x++) {
      ...
      len += sprintf(out + len, "\x1b[48;2;%i;%i;%im ",
                     (int)round(255 * ((double)x / (double)b->w)),
                     (int)round(255 * ((double)y / (double)b->h)),
                     (int)round(255 * ((double)x * (double)y /
                                       ((double)b->w * (double)b->h))));
...
int main() {
  ...
  ui_add(1, 1, u.ws.ws_col, u.ws.ws_row, 0, NULL, 0, draw, NULL, NULL, NULL,
         NULL, &u);

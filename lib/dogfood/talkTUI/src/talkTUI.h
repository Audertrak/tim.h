...
// TODO: Platform-specific headers (POSIX) - Need Windows equivalents
#ifdef _WIN32
#include <windows.h>
#else
#include <sys/ioctl.h> // For ioctl (getting window size)
#include <termios.h>   // For terminal control (raw mode, etc.)
#include <unistd.h>    // For read, ioctl, STDIN_FILENO, STDOUT_FILENO
#endif
...
/* original code uses read() from unistd.h (POSIX)
 * while ((n = read(STDIN_FILENO, buf, sizeof(buf))) > 0)
 *
 * replaced read() with ui_read_input() which will serve as a platform
 * abstraction
 *
 * TODO: implement ui_read_input()
 */
#define ui_loop(u)                                                             \
  char buf[64];                                                                \
  int n;                                                                       \
  while ((n = ui_read_input(buf, sizeof(buf), u)) > 0)

#define ui_update(u) _ui_update(buf, n, u)

#define ui_get(id, u) ((u)->b.data[id])

// TODO: I am not sure this is actually a problem, gemini just appears to think
// so
...
typedef struct ui_t {
  // TODO: Platform-specific: termios for terminal control (POSIX)
  struct termios tio;
  // TODO: Platform-specific: winsize for window size (POSIX)
  struct winsize ws;
  vec_box_t b;
  vec_evt_t e;
  ui_box_t *click;
...
void ui_new(int s, ui_t *u) {
  ...
  // TODO: Platform-specific: ioctl with TIOCGWINSZ to get window size (POSIX)
  ioctl(STDOUT_FILENO, TIOCGWINSZ, &(u->ws));

  // TODO: Platform-specific: tcgetattr and tcsetattr for terminal attributes
  // (POSIX)
  tcgetattr(STDIN_FILENO, &(u->tio));
  raw = u->tio;
  // TODO: Platform-specific: Setting raw mode (disabling ECHO and ICANON)
  // (POSIX)
  ...
  // TODO: Platform-specific: ANSI escape codes for terminal setup (including
  // mouse)
...
// windows shim of the read() fn in POSIX targets
int ui_read_input(char *buf, int buf_size, ui_t *u) {
#ifdef _WIN32
...
void ui_free(ui_t *u) {
  ...
  // TODO: Platform-specific: ANSI escape codes for restoring terminal state
  printf("\x1b[0m\x1b[2J\x1b[?1049l\x1b[?1003l\x1b[?1015l\x1b[?1006l\x1b[?25h");
  // TODO: Platform-specific: Restoring original terminal attributes (POSIX)
  ...
  vec_deinit(&(u->e));
  // TODO: Platform Specific - Checking Terminal via getenv (POSIX)
  term = getenv("TERM");
...
int ui_add(int x, int y, int w, int h, int screen, char *watch, char initial,
           func draw, func onclick, func onhover, void *data1, void *data2,
           ui_t *u) {
...
void ui_draw_one(ui_box_t *tmp, int flush, ui_t *u) {
  ...
  while (tok != NULL) {
    if (tmp->x > 0 && tmp->x < u->ws.ws_col && CURSOR_Y(tmp) > 0 &&
      ...
      // TODO: Platform-specific: ANSI escape code to position cursor
...
void ui_draw(ui_t *u) {
  ui_box_t *tmp;
  int i;
  // TODO: Platform-specific: ANSI escape codes for clearing screen
...
void _ui_update(char *c, int n, ui_t *u) {
  ...
  int ind, x, y;
  char cpy[n], *tok;
  // TODO: Platform-specific: Parsing ANSI escape codes for mouse events
  if (n >= 4 && c[0] == '\x1b' && c[1] == '[' && c[2] == '<') {
    ...
    case '6':
      if (u->canscroll) {
        ...
        // TODO: Platform-specific: ANSI escape codes
        printf("\x1b[0m\x1b[2J");
        ui_draw(u);
      }
      break;
 ...
 * HELPERS
 */
void _ui_text(ui_box_t *b, char *out) { sprintf(out, "%s", (char *)b->data1); }

int ui_text(int x, int y, char *str, int screen, func click, func hover,
            ui_t *u) {
